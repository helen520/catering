...
(function(window) {
	...
	var autoComplete = function(o) {
		...
		var handler = (function() {
			...
			};/* 为每个选择的dom都创建一个相对应的对象，这样选择多个dom时可以很方便地使用 */
			handler.prototype = {
				e : null,
				o : null,
				timer : null,
				...
				init : function(e, o) {/* 设置初始对象 */
					this.e = e, this.o = o, this.input = this.e
							.getElementsByTagName(this.o.input)[0],
							this.popup = this.e
									.getElementsByTagName(this.o.popup)[0],
					...
					for ( var i in $source) {
						if (value.length > 0) {
							...
							if (index > 0) {
								...
								li.innerHTML = '<a objectId=' + $source[i].id
										+ ' href="javascript:;">'
										+ $source[i].name + '</a>';
				...
				initEvent : function() {/* 各事件的集合 */
					var that = this;
					$source = that.o.source;
					this.input.onfocus = function() {
						if (this.inputValue)
							this.value = this.inputValue;
						var value = this.value, quickExpr = RegExp('^' + value,
								'i'), self = this;
						...
						that.timer = setInterval(function() {
							if (value != self.value) {/* 判断输入内容是否改变，兼容中文 */
								...
								if (value != '') {
									...
									if (that.o.source)
										that.match(quickExpr, value,
												that.o.source);
									else if (that.o.ajax)
										that.fetch(that.o.ajax, value,
												quickExpr);
								}
							}
						}, 200);
					...
					this.input.onblur = function() {/* 输入框添加事件 */
						...
						var aClick = function() {
							that.input.inputValue = this.innerText;
							that.input.objectId = this.getAttribute("objectId");
							that.popup.innerHTML = '';
						...
						var aKeydown = function(event) {
							...
							if (current === len && event.keyCode === 9) {/* tab键时popup隐藏 */
								...
								if (current > len) {
									...
									that.popup.getElementsByTagName('a')[current]
											.focus();
								}
			...
			handler.prototype.init.prototype = handler.prototype;/*
																	 * JQuery
																	 * style，这样我们在处的时候就不用每个dom元素都用new来创建对象了
																	 */
