...
function initDeskView() {
	...
	$('#functionMenuButton').click(functionMenuButtonClick);
	...
	if (dishOrder) {
		...
		for ( var i in orderItems) {
			...
			var orderItemDiv = $('<div>').addClass("button orderItem").text(
					orderItem.dishName).click(function() {
				if ($isDesktop) {
...
function _changeDeskButtonClick() {
	...
	var targetDesk = null;
	var changeDeskOkButton = $('<div>').addClass("dishOrderCmdButton").text(
	...
	var changeDeskDialog = $(dialogDiv).modal();
...
function _mergeDishOrderButtonClick() {
	...
	var targetDesk;
	var mergeDishOrderOKButton = $('<button>').addClass("dishOrderCmdButton")
			.text($.i18n.prop('string_bingDan')).click(
					mergeDishOrderOKButtonClick).appendTo(bottomDiv);
	...
	var mergeDishOrderDialog = $(dialogDiv).modal();
...
function functionMenuButtonClick() {
	...
	if ($storeData.employee.job === '店长' || $storeData.employee.job === '收银') {
		...
		$("<a>").css("text-decoration", "none").addClass(
				...
				"../reporting/classReporting?storeId=" + $storeId).attr(
				"target", "_blank").text($.i18n.prop('string_caiWuBaoBiao'))
				.appendTo(reportFieldset);
	...
	if (isDemo()) {
		var salesDataResetBtn = $("<div>").addClass(
				"ui-radius ui-shadow functionMenuButton").text("清空销售数据")
				.appendTo(systemFieldset);
		salesDataResetBtn.click(function() {
			showSalesDataResetDialog();
			functionMenuPanelDialog.close();
		});
	}
...
function showSalesDataResetDialog() {
	showConfirmDialog("！！！危险！！！",
			"您确定要删除所有该店的订单，预订，自助点餐，会员信息，已发放的优惠券，付款纪录吗？<p>该操作不会删除店铺的配置信息</p>",
			function() {
				showLoadingDialog('正在清除...');
				$.ajax({
					type : 'POST',
					url : '../storeData/salesDataReset',
					data : {
						storeId : $storeId,
					},
					...
					error : function(error) {
						hideLoadingDialog();
						showAlertDialog("清除失败", error);
					},
					success : function(isOK) {
						hideLoadingDialog();
						if (isOK)
							showAlertDialog("清除成功", "点击确定刷新页面", function() {
								location.reload();
							});
						else {
							showAlertDialog("清除失败！")
						}
					}
				});
			}, $storeId);
